    <?php
    //include "authentification.inc.php";
    ...
    function creePost($newPostName, $salleVal, $typePost, $idUser){
      ...
      try {
            ...
            $newPostIndIP = $namePostt['indIP'];

            $totPost = getIdPost();
            $totPost = "p".intval($totPost['COUNT(nPoste)']+1);

            $req = $cnx->prepare("insert into poste (nPoste, nomPoste, indIP, ad, typePoste , nSalle)
                values(:totPost,:newPostName,:newPostIndIP,:idUser,:typePost,:salleVal)");

            $req->bindValue(':totPost', $totPost, PDO::PARAM_STR);

            $req->bindValue(':newPostName', $newPostName, PDO::PARAM_STR);

            $req->bindValue(':newPostIndIP', $newPostIndIP, PDO::PARAM_STR);
